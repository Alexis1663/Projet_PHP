...
class MdlAdmin
{
    ...
    public function addArticle()
    {
        ...
        $contenu = Validation::cleanString($_REQUEST['contenu']);
        $dVueErreur = Validation::val_form_ajout_article($titre, $contenu);
        if (empty($dVueErreur)) {
            if (isset($_FILES['image']) && !empty($_FILES['image']['name'])) {
                ...
                if ($move) {
                    $adminG->addA(date('Y-m-d'), $_REQUEST['titre'], $_REQUEST['contenu'], date('Y-m-d') . "_" . $_REQUEST['titre'] . "." . $extensionUpload, 0);
